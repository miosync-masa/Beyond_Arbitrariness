...
%% The "S" prefix makes two-digit section numbers (S10, S11) overrun the default
%% ToC number box, producing "S10ASJP" / "S11Computational". Widen the box.
\makeatletter
\renewcommand*\l@section[2]{\@dottedtocline{1}{1.5em}{3.2em}{#1}{#2}}
\makeatother
...
Table~\ref{tab:s8} pairs the two likelihoods; Figure~\ref{fig:si-binom} compares their
...
per-bin Binomial predictive output was stored, Figure~\ref{fig:si-binom} compares summary
statistics only and does not reconstruct a distribution from summaries.

\input{tables/tableS8}
...
robustness sweep in Table~\ref{tab:s9} and Figure~\ref{fig:si-coverage} varies the minimum
...
\input{tables/tableS10}
\input{tables/tableS11}
\clearpage
...
underprediction across both databases (Table~\ref{tab:s13} and Figure~\ref{fig:si-family}).
...
\input{tables/tableS17}
\input{tables/tableS18}
\clearpage
